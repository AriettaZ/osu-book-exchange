...
# Edit: 7/24 Gail added a job that declines expired contract.
# Description: Makes actions corresponding to the received request related to contracts.
    ...
    respond_to do |format|
      if @contract.save
        ...
        if @contract.status == "waiting"
          ...
          # Decline the expired contract if it is not confirmed or declined by the expiration time.
        ...
        # If contract is confirmed, then post is closed(3) and an email is sent to users.
          ...
          # Three days after the meeting time, set the order to completed if no user have set it.
